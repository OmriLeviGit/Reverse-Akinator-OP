...
const ArcSelection: React.FC<ArcSelectionProps> = ({ selectedArc, setSelectedArc, availableArcs }) => {
  // Auto-select "All" if no selection and arcs are available
  useEffect(() => {
    if (availableArcs.length > 0 && !selectedArc) {
      setSelectedArc("All");
    }
  }, [availableArcs, selectedArc, setSelectedArc]);
  ...
  return (
    <div className="space-y-3">
      ...
      <Select value={selectedArc || "All"} onValueChange={setSelectedArc}>
