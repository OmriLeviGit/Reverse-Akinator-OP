...
class CharacterProcessor:
    ...
    def process_single_character(self, character_info: dict, verbose=False, retry_count=0) -> dict:
        ...
        try:
            # Step 1: Scrape all character data
            ...
            total_paragraphs = sum(len(paragraphs) for paragraphs in character_data.get('narrative_sections', {}).values())
            avatar_found = bool(character_data.get('avatar_url'))
            affiliations_found = len(character_data.get('affiliations', '').split(';')) if character_data.get('affiliations') else 0
            
            print(f"  Found: {structured_fields} structured fields, {narrative_sections} narrative sections")
            print(f"  Found: {total_paragraphs} total paragraphs, avatar: {avatar_found}, affiliations: {affiliations_found}")
    ...
    def process_characters_batch(self, characters: list[dict], start_index: int = 0, 
        ...
        for i in range(start_index, end_index):
            ...
            # Delay between characters to be respectful to the wiki
            if i < end_index - 1:  # Don't delay after the last character
                delay_time = self.delay_between_characters + random.uniform(0.5, 1.5)
    ...
    def _print_progress_summary(self):
        ...
        if self.stats['403_blocks_encountered'] > 0:
            print(f"403 Blocks: {self.stats['403_blocks_encountered']} | Characters Retried: {self.stats['characters_retried']}")
        print("---")
    ...
    def _print_final_summary(self, results: dict):
        ...
        print(f"  Small Avatars Created: {stats['small_avatars_created']}")  
